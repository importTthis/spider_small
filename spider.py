...
from settings import MAX_FAILED_COUNT, NEED_PROXY, MYSQL_TABLE
...
class SpiderFather:
    ...
    def start(self):
        request_queue = TianMaoRequest(url=self.url.get(self.sign), callback=self.parse_index, need_proxy=NEED_PROXY,
                                       headers=self.headers.get(self.sign),
                                       params=self.params.get(self.sign))
...
class TianMaoSpider(SpiderFather):
    ...
    url = {
        "small": "https://rate.tmall.com/list_detail_rate.htm",
        "taobao": 'https://rate.taobao.com/feedRateList.htm',
        "jd": 'https://club.jd.com/comment/productPageComments.action'
    }
    ...
    resp = {
        "small": "response.content.decode('utf-8')[12:-1]",
        "taobao": "response.content.decode('utf-8')[12:-1]",
        "jd": "req = response.text[20:-2]"
    }

    result = {
        "small": 'req.get("rateDetail").get("rateList")',
        "taobao": 'req.get("comments")',
        "jd": 'req.get("comments")'
    }
...
self.res = {
    ...
    page = {
        "small": 'paginator.get("page")',
        "taobao": 'req.get("currentPageNum")',
        "jd": 'req.get("maxPage")'
    }

    last_page = {
        "small": 'paginator.get("lastPage")',
        "taobao": "req.get('maxPage')",
        "jd": "self.params['page']"
    }
    def parse_index(self, response):

        req = eval(self.resp.get(self.sign))
        req = json.loads(req)

        results = eval(self.result.get(self.sign))

        for result in results:
            exec_sql = self.yield_dic.get(self.sign)
            exec(exec_sql)
            yield self.res

        paginator = req.get('rateDetail').get("paginator")
        page = eval(self.page.get(self.sign))
        last_page = eval(self.last_page.get(self.sign))
        ...
        if page < last_page:
            ...
            if self.sign == "jd":
                fil = "page"
            elif self.sign == "small":
                fil = "currentPage"
            else:
                fil = "currentPageNum"

            params[self.sign][fil] = int(page) + 1

            request = TianMaoRequest(url=self.url.get(self.sign), callback=self.parse_index, need_proxy=NEED_PROXY,
                                     headers=self.headers.get(self.sign),
                                     params=params.get(self.sign))
            yield request
